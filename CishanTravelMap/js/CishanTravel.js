...
$(document).bind("mobileinit", function(){
    ...
    $.mobile.notesdb.transaction(function (t) {
		...
		// ------------------------------------------------------------------
		// 第一次載入預設的 /js/Travel.js 中的 json 資料
		if (isLoadData==null){  // 第一次載入預設的資料 
			localStorage.setItem("loaddata",true); // 第二次以後不載入預設的資料	
  			// 以 js/TravelData.js 建立資料庫
...
if (FoodNote.lat==null || FoodNote.lng==null ){ //若清空資料庫後中心點仍舊，則將判斷式改為「！＝」後重整即可
	FoodNote.lat=22.884841;  // 開始時，以火車頭當歸鴨為中心點
	FoodNote.lng=120.482118;
...
function insertItem(e) { // 新增一個景點資料
	...
	if (title==""){
		...
		$.mobile.notesdb.transaction(function(t) {
			t.executeSql('INSERT into Food(title, details, created, latitude, longitude , pic) VALUES (?,?,date("now"),?,?,?);',
			[title, details, lat, lng , pic],
			function() {
...
function updateItem(e) {  // 修改指定 id 的資料
	...
	var pic = $("#pic2").val();
	$.mobile.notesdb.transaction(function(t) {
		t.executeSql('UPDATE Food SET title = ?, details = ?, updated = date("now"), latitude=?, longitude=?,pic=? WHERE id = ?',
		    [title, details, latitude, longitude, pic, id],  /*增加修改圖片網址功能*/
